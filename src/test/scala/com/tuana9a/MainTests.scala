...
import com.tuana9a.learn.spark.udafs.TestAggs.{CaseClass2, CaseClass3, CaseClass4}
import com.tuana9a.learn.spark.udafs.{HighestSalaryAgg, TestAggs}
import org.apache.spark.sql.{DataFrame, Dataset, Encoder, SparkSession}
import org.apache.spark.sql.functions.{count, first, lit, max, sum, udaf, when}
import org.junit._
...
class MainTests {
  ...
  def test0(): Unit = {
    try {
      ...
      config1.load(new FileInputStream("test.properties"))
  ...
  def test1(): Unit = {
    ...
    class Base[T] {
      ...
      def getThis(): Base[T] = {
        println(this.asInstanceOf[T])
        this
      }
    }

    class Impl1 extends Base[Int] {
  ...
  case class Temp1(id: Int, age: Int)
  ...
  def test9(): Unit = {
    ...
    Seq(1, 2, 3, 1, 2, 3, 1, 2, 3, 3, 4, 5, 1, 1, 2, 4, 5)
      .toDF("id")
      .withColumn("age", lit(12))
      .groupBy($"id")
      .agg(count("*").as("count"))
      .show()
    ...
    Seq(1, 2, 3, 1, 2, 3, 1, 2, 3, 3, 4, 5, 1, 1, 2, 4, 5)
      .toDF("id")
      .withColumn("age", lit(12))
      .filter($"id".isin(list: _*))
      .show()


    val d1 = Seq(1, 2, 3, 1, 2, 3, 1, 2, 3, 3, 4, 5, 1, 1, 2, 4, 5)
      .toDF("id")
      .withColumn("age", lit(12))
      .as[Temp1]
...
//    val d2 = spark.emptyDataset[Temp2]
//    Seq(Temp2(10,10),Temp2(1,2),Temp2(3,4)).toDS().show()

  }
//  case class Temp2(id: Int, age: Int)
